class CanvasHelper {
  ...
  clicked() {
    ...
    const easeIn = () => {
      if (this.clickedTimer < 1800) {
        this.clickedTimer += this.clickedTimer / 16;
  ...
  draw() {
    ...
    this.nodes.forEach((node, i) => {
      if (this.ctx) {
        ...
        this.ctx.strokeStyle = this.makeColor(node.ordinal, 20);
        this.ctx.moveTo(node.x, node.y + (this.wheelValue * -1.5));
        this.ctx.lineTo(node.x + (i / 10), node.y + 45 + (this.wheelValue * 4));
        this.ctx.stroke();
        this.ctx.closePath();
        ...
        // this.ctx.fill();
        const rand = Math.round(Math.max(1, Math.floor(Math.random() * 0)));
        if (node.x > x + this.clickedTimer) node.x -= ((node.x / node.rotation) * speed) * rand;
        if (node.x < x + this.clickedTimer) node.x += ((node.x / node.rotation) * speed) * rand;
        if (node.y > y + this.clickedTimer) node.y -= ((node.y / node.rotation) * speed) * rand;
        if (node.y < y + this.clickedTimer) node.y += ((node.y / node.rotation) * speed) * rand;
        if (!this.isMouseDown) {
...
function bricks(node: HTMLCanvasElement) {
